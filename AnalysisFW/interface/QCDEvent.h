...
class QCDEvent
{
    public:
      ...
      void setGenJets(const vector<QCDGenJet>& fQCDGenJets); // this is LLR GenJets, QCDGenJet now has angularities or any other thing we want.

//      void setGenJets(const vector<LorentzVector>& fGenJets); //this is the old SMPJ way of doing things
      ...
      unsigned int nQCDGenJets()                        const {return QCDGenJets_.size();}
//      unsigned int nGenJets()                          const {return GenJets_.size();}
      ...
      const QCDGenJet&     genjet(int i)               const {return QCDGenJets_[i];}
