...
def producer(RunYear,era,jettype,Mode):
  ...
  with open(fname, 'w') as f:
    ...
    for line in inputs:
      f.write(line+'\n')
    # Jet collections
    f.write("jetname='slimmedJetsPuppi"+("AK8" if jettype=='ak8' else "")+"'\n")
...
#      f.write("process.load('RecoJets.JetProducers.JetAngularities_cfi')\n")
#      f.write("process.JetAngularities.srcJets = cms.InputTag(jetname)\n")
#      f.write("process.JetAngularities.jetsLabel  = cms.string('QGL_AK4PFchs')\n\n")
...
#      f.write("process.JetAngularities*\n                        ")
    if Mode=="dt":
